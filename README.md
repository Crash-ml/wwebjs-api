...
### Outbound Proxy
The Chromium instance that powers each session can be routed through an outbound proxy by setting the `PROXY_URL` environment variable (e.g. `http://10.0.0.10:8118`, `https://...`, or `socks5://...`). If the proxy requires basic authentication, set both `PROXY_USERNAME` and `PROXY_PASSWORD`; otherwise leave them unset. When `PROXY_URL` is empty, sessions connect directly with no behavior change.
